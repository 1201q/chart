...
export function useCandleChart(options: UseChartOptions) {
  ...
  const tickerTradePrice = useTickerSelector2(options.code, (t) => t?.tradePrice ?? 0);
  const tickerTimestamp = useTickerSelector2(options.code, (t) => t?.timestamp ?? 0);
  ...
  useEffect(() => {
    ...
    const tickerTime = tickerTimestamp;
  ...
  }, [tickerTradePrice, tickerTimestamp, timeframe]);
