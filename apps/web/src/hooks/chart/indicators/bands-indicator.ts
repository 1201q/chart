...
const defaults: InternalBandsOptions = {
...
export class BandsIndicator extends PluginBase implements ISeriesPrimitive<Time> {
  ...
  calculateBands() {
    const { mode, period, k, envelopePercent } = this._options;

    if (!this._seriesData.length || period <= 0) {
      this._bandsData = [];
    ...
    if (mode === 'bollinger') {
      ...
      for (let i = period - 1; i < closes.length; i++) {
        ...
        const mean = sum / period;
        const upper = mean * (1 + pct);
        const lower = mean * (1 - pct);
