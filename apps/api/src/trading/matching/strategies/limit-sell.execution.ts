 ...
 */
@Injectable()
export class LimitSellExecution extends BaseExecutionStrategy {
  private readonly logger = new TradingLogger(LimitSellExecution.name);
  ...
  async execute(
    ...
    this.logger.log(
      `LIMIT SELL filled: ${matchResult.totalFilled} (${matchResult.fills.length} fills)`,
    );
